...
+ (void)setShadowOpacity:(CGFloat)shadowOpacity{
    [self sharedView].shadowView.layer.shadowOpacity = shadowOpacity;
}

//+ (void)setShadowRadius:(CGFloat)shadowRadius{
//    [self sharedView].hudView.layer.shadowRadius = shadowRadius;
//}

+ (void)setShadowOffset:(CGSize)shadowOffset{
    [self sharedView].shadowView.layer.shadowOffset = shadowOffset;
}

+ (void)setShadowColor:(nonnull UIColor *)shadowColor{
    [self sharedView].shadowView.layer.shadowColor = shadowColor.CGColor;
}
...
- (instancetype)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:frame])) {
        ...
        //successImage = [UIImage imageNamed:@"Failed"];
...
- (void)updateHUDFrame {
    ...
    self.shadowView.bounds = self.hudView.bounds;
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000
...
- (void)moveToPoint:(CGPoint)newCenter rotateAngle:(CGFloat)angle {
    ...
    self.shadowView.transform = CGAffineTransformMakeRotation(angle);
    if (self.containerView) {
    ...
    self.shadowView.center = self.hudView.center;
}
...
//        self.shadowView.transform = CGAffineTransformScale(self.hudView.transform, 1.3, 1.3);
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            __block void (^animationsBlock)(void) = ^{
                ...
                strongSelf.shadowView.transform = CGAffineTransformScale(strongSelf.hudView.transform, 1/1.3f, 1/1.3f);
                ...
                if(self.defaultStyle != SVProgressHUDStyleCustom && greateriOS9){
                    ...
                    strongSelf.shadowView.alpha = 0.0f;
...
- (UIView *)shadowView{
    if (!_shadowView){
        ...
        _shadowView.layer.shadowOffset = CGSizeMake(0, 13);
        _shadowView.layer.shadowColor = HexRGBAlpha(0x000, .6).CGColor;
    _shadowView.layer.shadowRadius = 15;
        
        _shadowView.layer.masksToBounds = NO;
        _shadowView.clipsToBounds = NO;
    
    if(!_shadowView.superview){
        [self addSubview:_shadowView];
        
    }

    [self insertSubview:_shadowView belowSubview:self.hudView];
   // [self insertSubview:_shadowView aboveSubview:self.backgroundView];
    return _shadowView;
}
...
#warning --------------改变textcolor
    ...
    _statusLabel.textColor = colorTextMain;
...
#warning -------------------把这个效果去了之后，label的字体显示正确
...
- (void)setShadowOpacity:(CGFloat)shadowOpacity{
    if (!_isInitializing) _shadowOpacity = shadowOpacity;
}
    
//- (void)setShadowRadius:(CGFloat *)shadowRadius{
//    if (!_isInitializing) _shadowRadius = shadowRadius;
//}
    
- (void)setShadowOffset:(CGSize *)shadowOffset{
    if (!_isInitializing) _shadowOffset = shadowOffset;
}
